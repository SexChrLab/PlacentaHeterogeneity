...
configfile: "process_dna_config.json"
...
rule all:
    # Further processing
    input:
        expand("vqsr/chr{chr}.gatk.called.vqsr.sv.biallelic.snp.{sample}.het.vcf", chr=chr_current, sample=config["all_dna_samples_without_SM"])
    input:
        expand("vqsr/chr{chr}.gatk.called.vqsr.sv.biallelic.snp.{sample}.vcf", chr=chr_current, sample=config["all_dna_samples_without_SM"])
    # VQSR on chr8
    input:
    ...
    input: #picard
        expand("processed_bams/dna/{sample}.GRCh38.p12.genome.XXonly.mkdup.sorted.mkdup.merged.bam", sample=config["all_dna_samples"]),
        expand("processed_bams/dna/{sample}.GRCh38.p12.genome.XXonly.mkdup.sorted.mkdup.merged.bam.bai", sample=config["all_dna_samples"]),
        expand("picard_stats/{sample}.GRCh38.p12.genome.XXonly.picard_mkdup_metrics.txt", sample=config["all_dna_samples"])
    input: #merge bams
        expand("processed_bams/dna/{sample}.GRCh38.p12.genome.XXonly.mkdup.sorted.merged.bam", sample=config["all_dna_samples"])
...
rule map_and_process_trimmed_reads_dna:
	input:
		...
		fai_xx = "/scratch/tphung3/PlacentaSexDiff/A_placenta/batch_1_exome_processing/refs/GRCh38.p12.genome.XXonly.fa.fai",
		ref_xx = "/scratch/tphung3/PlacentaSexDiff/A_placenta/batch_1_exome_processing/refs/GRCh38.p12.genome.XXonly.fa"
	output:

...
# Make a list of ids for each tissue
...
if not os.path.exists(args.outfile):
    header = ['tissue', 'wes_fn', 'rna_fn', 'wes_numvar', 'rna_numvar', 'wes_subset_numvar', 'rna_subset_numvar', 'wes_subset_median', 'rna_subset_median', 'shared_numvar', 'wes_subset_shared_median', 'rna_subset_shared_median', 'wes_subset_unique_numvar', 'rna_subset_unique_numvar', 'wes_subset_unique_median', 'rna_subset_unique_median']
...
for wes_file in [f for f in os.listdir(WES_FILES_DIR) if f.endswith('_allele_balance.tsv')]: #OBG0111_OBG0111-2-011_chr8_allele_balance.tsv from wes vs OBG0111-2-011_RNA_OBG0111-2-011_chr8_allele_balance.tsv from rnaseq
    items = wes_file.split('_')
    if items[1] in rna_ids:
        out = [tissue]
        rna_name = wes_file.split('_')[1].split('-SM-')[0]
